...
class TransferSubsystem(hardwareMap: HardwareMap) : Subsystem() {
    ...
    companion object {
        @JvmField
        var DEFAULT_POSITION = 0.2639

        @JvmField
        var TRANSFER_POSITION = -1.0
    }
    ...
    fun adjustPosition(delta: Double): Closure = exec {
        ...
        state = State.AtPosition((current + delta).coerceIn(-1.0, 1.0))
    }

    fun transfer(): Closure = exec { state = State.AtPosition(TRANSFER_POSITION) }
    fun reset(): Closure = exec { state = State.Default }
    fun setPosition(position: Double): Closure = exec { state = State.AtPosition(position.coerceIn(-1.0, 1.0)) }
